...
INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t=CSR(imm) ; CSR(imm)=t       ; R(rd)=t);
// INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, );

INSTPAT("0011000 00010 00000 000 00000 11100 11", mret , R, s->dnpc = cpu.csrs.mepc);
INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, cpu.csrs.mepc=s->dnpc;cpu.csrs.mcause = R(15);s->dnpc = cpu.csrs.mtvec;printf("%08x : 写入异常号 ： %08x\n",s->pc,R(15));s->dnpc = cpu.csrs.mtvec);
