...
// B-type 指令
// 原代码中条件跳转的目标地址计算正确，不需要修改
INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B, s->dnpc = (src1 != src2) ? s->pc + imm : s->dnpc); 
INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B, s->dnpc = (src1 >= src2) ? s->pc + imm : s->dnpc); 
...
// R-type 指令
// 原代码：R(rd) = src1 + src2;printf("test: %lu\n",SEXT(1,32))
// 修改：移除调试打印
INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add, R, R(rd) = src1 + src2);

INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub, R, R(rd) = src1 - src2);

// 原代码：R(rd) = (int32_t)src1%(int32_t)src2
// 修改：添加除零检查
INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem, R, R(rd) = src2 != 0 ? (int32_t)src1%(int32_t)src2 : src1); 
...
// 修改：简化实现，直接使用SEXT
INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl, R, R(rd) = src1 >> SEXT(src2,5));
